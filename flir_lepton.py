...
# Based on: https://github.com/Kheirlb/purethermal1-uvc-capture/tree/master/python

# USB Permissions: # TODO: Can this command be simplified?
# sudo sh -c "echo 'SUBSYSTEMS==\"usb\", ATTRS{idVendor}==\"1e4e\", ATTRS{idProduct}==\"0100\", SYMLINK+=\"pt1\", GROUP=\"usb\", MODE=\"666\"' > /etc/udev/rules.d/99-pt1.rules"
# Otherwise, the script needs to be run as root!

# Check Micro-USB cable if it can transfer data and not only power

# TODO: Set proper path to libuvc.so
...
class FlirLepton:
    ...
    def perform_manual_ffc(self, device_handle):
        ...
        runFFC = 0x42
        controlID = (runFFC >> 2) + 1
        print('controlID: ' + str(controlID))
        set_extension_unit(device_handle, SYS_UNIT_ID, controlID, shutter_mode, sizeData)

    # Source: https://github.com/Kheirlb/purethermal1-uvc-capture
    # 0=manual, 1=auto, 2=external
    def set_ffc(self, devh, ffc_type):
        sizeData = 32
        getSDK = 0x3D
        controlID = (getSDK >> 2) + 1
        print('controlID: ' + str(controlID))
        sysShutterManual = lep_sys_shutter_mode(ffc_type, 0, 1, 0, 0, 180000, 1, 0, 150)
        set_extension_unit(devh, SYS_UNIT_ID, controlID, byref(sysShutterManual), sizeData)

    # Source: https://github.com/Kheirlb/purethermal1-uvc-capture
    # 0=HIGH, 1=LOW, 2=AUTO
    def set_gain_mode(self, devh, gainmode):
        sizeData = 4
        gain_mode = (c_uint16)(gainmode)
        setGainSDK = 0x49
        controlID = (setGainSDK >> 2) + 1
        print('controlID: ' + str(controlID))
        set_extension_unit(devh, SYS_UNIT_ID, controlID, byref(gain_mode), sizeData)
        self.perform_manual_ffc(devh)

    # Based on: https://github.com/groupgets/GetThermal/blob/bb467924750a686cc3930f7e3a253818b755a2c0/src/dataformatter.cpp#L6 and https://github.com/Kheirlb/purethermal1-uvc-capture
    def generate_colour_map(self):
        ...
        def chunk(ulist, step): return map(
    ...
    def __update(self):
        ...
        res = libuvc.uvc_init(byref(context), 0)
        if res < 0:
            print("uvc_init error")
            exit(1)

        try:
            res = libuvc.uvc_find_device(context, byref(device), PT_USB_VID, PT_USB_PID, 0)
            if res < 0:
            ...
            try:
                ...
                print_device_formats(device_handle)

                frame_formats = uvc_get_frame_formats_by_guid(device_handle, VS_FMT_GUID_YUYV)
